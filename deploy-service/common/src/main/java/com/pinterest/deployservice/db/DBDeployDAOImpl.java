...
public class DBDeployDAOImpl implements DeployDAO {
    ...
    private static final String INSERT_DEPLOYMENT_TEMPLATE = "INSERT INTO deploys SET %s";
    ...
    private static final String GET_DEPLOYMENT_BY_ID = "SELECT * FROM deploys WHERE deploy_id=?";
    private static final String DELETE_DEPLOYMENT = "DELETE FROM deploys WHERE deploy_id=?";
    private static final String GET_ALL_DEPLOYMENTS_TEMPLATE = "SELECT * FROM deploys %s";
    ...
    public DeployQueryResultBean getAllDeploys(DeployQueryFilter filterBean) throws Exception {
        ...
        if (StringUtils.isNotEmpty(filterBean.getFilter().getCommit())) {
            ...
            queryStr =
                    String.format(
                            GET_ALL_DEPLOYMENTS_WITH_COMMIT_TEMPLATE, filterBean.getWhereClause());
            if (filterBean.getWhereClause().contains("LIMIT")) {
                String[] separateByLimit = filterBean.getWhereClause().split("LIMIT");
                queryCountStr =
                        String.format(
                                GET_COUNT_FOR_ALL_DEPLOYMENTS_WITH_COMMIT_TEMPLATE,
                                separateByLimit[0]);
            } else {
                queryCountStr =
                        String.format(
                                GET_COUNT_FOR_ALL_DEPLOYMENTS_WITH_COMMIT_TEMPLATE,
                                filterBean.getWhereClause());
            }
        } else {
            filterBean.generateClauseAndValues();
            ...
            if (filterBean.getWhereClause().contains("LIMIT")) {
                String[] separateByLimit = filterBean.getWhereClause().split("LIMIT");
                queryCountStr =
                        String.format(GET_COUNT_FOR_ALL_DEPLOYMENTS_TEMPLATE, separateByLimit[0]);
            } else {
                queryCountStr =
                        String.format(
                                GET_COUNT_FOR_ALL_DEPLOYMENTS_TEMPLATE,
                                filterBean.getWhereClause());
            }
        }
        ...
        try {
            ...
            long total =
                    run.query(
                            connection,
                            queryCountStr,
                            SingleResultSetHandlerFactory.newObjectHandler(),
                            newArr);
            long maxToReturn =
    ...
    public boolean isThereADeployWithBuildId(String buildId) throws Exception {
        return new QueryRunner(dataSource)
                        .query(
                                "SELECT EXISTS(SELECT * FROM deploys WHERE build_id =?)",
